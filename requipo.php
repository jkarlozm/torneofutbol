    ...
    <head>
        ...
        <script>			
			function opciones(val)
			{
				...
				if(opcion==1)
				{
					window.location= "muestra_equipo.php?id="+val;
				}
				else if (opcion==2)
				{
					window.location= "modificar_equipo.php?id="+val;
				}				
				else if (opcion==4)
				{
					window.location= "edita_fotos_equipo.php?id="+val;
				}
		...
		<div id="page">
			<div id="content-other">
				<div class="text-right">
					<a href="form_equipos.php" class="btn btn-primary">Agregar Equipos</a>
				</div>	
				<?php
				...
				<div class="datagrid">
						<table class="table">
			                ...
			                <tbody>
								<?php									
									$productosq=mysql_query("select * from equipos");								
									if(mysql_num_rows($productosq)>0)
										while($productosf=mysql_fetch_assoc($productosq)) { ?>
			                    			<tr>
			                    				<td class=" ">									
													<?php 
														if (file_exists("equiposimagenes/".$productosf['logotipoEquipo'])==1) { ?>
			                                 				<img width="120px" height="100px" src="<?php echo "equiposimagenes/".$productosf['logotipoEquipo']?>">
														<?php }
														else { ?>
			                                 				<img class="img-responsive" src="images/0.jpg">
			                                 			<?php } ?>
												</td>
			                        			<td class=" "><?php echo $productosf['nombreEquipo'];?></td>
			                        			<td><?php echo get_campo("nombreCampo", "campos", "idCampo", $productosf['idCampoEquipo']) ?></td>
			                        
												<td>
													<select class="form-control" onchange="opciones(<?php echo $productosf['idEquipo'];?>)" id="opciones<?php echo $productosf['idEquipo'];?>">
